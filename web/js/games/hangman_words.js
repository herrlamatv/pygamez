/*
 * hangman_words.js - Ratewörter für das Galgenmännchen, je Sprache
 * (Port von games/hangman_words.py)
 * ================================================================
 ...
 * - PG.hangmanWords.wordsFor(lang, mode) liefert eine gefilterte, groß
 *   geschriebene Liste; mode wählt die Länge: "short" (3-5), "long" (7-12)
 *   oder "mixed" (3-12).
 */
(function () {
  "use strict";
  ...
  const RANGES = { short: [3, 5], long: [7, 12], mixed: [3, 12] };

  /** Groß geschriebene, auf A-Z und Modus-Länge gefilterte Wortliste. */
  function wordsFor(lang, mode = "mixed") {
    const [lo, hi] = RANGES[mode] || RANGES.mixed;
    const raw = WORDS[lang] || WORDS.en || [];
    const out = [];
    const seen = new Set();
    for (const w of raw) {
      const u = String(w).trim().toUpperCase();
      if (u.length >= lo && u.length <= hi && /^[A-Z]+$/.test(u) && !seen.has(u)) {
        seen.add(u);
        out.push(u);
      }
    }
    if (!out.length) {
      // Notnagel: englische Liste, Länge egal
      const en = WORDS.en.filter((w) => w.length >= lo && w.length <= hi);
      return en.length ? en : WORDS.en.slice();
    }
    return out;
  }

  PG.hangmanWords = { WORDS, wordsFor };
})();
