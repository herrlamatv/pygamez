 ...
 *
 *   swear      Wortfilter (Regex-Listen aller 14 Sprachen, unten eingebettet)
 *   ugc        Speicher der eigenen Bahnen - im Browser über PG.store statt
...
(function () {
  ...
  // ===========================================================================
  //  Eigene Bahnen speichern (ugc.py)
  // ===========================================================================
  //
  // Gespeichert wird unter PG.store "minigolf.ugc", aufgebaut wie ugc.json:
  //   {"v": 1, "author": "Lama", "minigolf": [ {map}, {map}, ... ]}
  // Eine Map ist das normale Bahn-Objekt plus Kopfdaten (id, name, author,
  // created, edited). Eine exportierte Datei enthält genau EINE Map:
  ...
  const ugc = (function () {
    ...
    const VERSION = 1;
    const MAX_MAPS = 60;
    const EXT = ".lamapgzmap";
    const FORMAT = "pygamez.minigolf.map";
    ...
    const MIN_W = 60.0, MAX_W = 160.0, STEP_W = 10.0;
    const MIN_H = 80.0, MAX_H = 240.0, STEP_H = 10.0;
    const MIN_PAR = 1, MAX_PAR = 9;
    ...
    function writeRaw(data) {
      ...
      if (Array.isArray(data.minigolf) && data.minigolf.length) out.minigolf = data.minigolf;
    ...
    function valid(m) {
      return !!(m && typeof m === "object" && validId(m.id) && typeof m.name === "string" && m.name.trim()
        && Array.isArray(m.tee) && Array.isArray(m.cup));
    }

    function loadMaps() {
      const items = readRaw().minigolf;
      if (!Array.isArray(items)) return [];
      return items.filter(valid).map((m) => gen.normalize(m));
    }

    function get(mapId) {
      return loadMaps().find((m) => m.id === mapId) || null;
    }

    const count = () => loadMaps().length;
    const isFull = () => count() >= MAX_MAPS;

    /** Freie id: hängt -2, -3 ... an, solange wanted schon vergeben ist. */
    function uniqueId(wanted, ignore) {
      const base = slug(wanted) || "map";
      const taken = new Set(loadMaps().filter((m) => m.id !== ignore).map((m) => m.id));
    ...
    /** Speichert eine Bahn. Gibt [ok, grund] zurück ("invalid"/"id"/"swear"/"full"). */
    function saveMap(m) {
      if (!valid(m)) return [false, "invalid"];
      if (!validId(m.id)) return [false, "id"];
      if (!swear.allClean(m.name, m.id, m.author)) return [false, "swear"];
      const data = readRaw();
      let items = Array.isArray(data.minigolf) ? data.minigolf : [];
      items = items.filter((x) => valid(x) && x.id !== m.id);
      if (items.length >= MAX_MAPS) return [false, "full"];
      m = gen.cloneHole(m);
      m.v = VERSION;
      m.edited = stamp();
    ...
    function deleteMap(mapId) {
      const data = readRaw();
      const items = data.minigolf;
      if (!Array.isArray(items)) return false;
      const keep = items.filter((x) => !x || x.id !== mapId);
    ...
    function newMap(name = "", mapId = "", author = "", hole = null) {
      const h = gen.normalize(hole ? gen.cloneHole(hole) : gen.makeHole(3, [gen.CW / 2, gen.CH - 16], [gen.CW / 2, 22]));
      const now = stamp();
      return Object.assign({ v: VERSION, id: mapId, name, author, created: now, edited: now }, h);
    }

    /** Text der .lamapgzmap-Datei. Gibt [ok, grund, text] zurück. */
    function exportText(m) {
      if (!valid(m)) return [false, "invalid", ""];
      if (!swear.allClean(m.name, m.id, m.author)) return [false, "swear", ""];
      const payload = { format: FORMAT, v: VERSION, app: "PyGameZ", exported: stamp(), map: gen.cloneHole(m) };
      return [true, "", JSON.stringify(payload, null, 2)];
    }

    /** Liest eine geteilte Bahn ein und speichert sie. Gibt [ok, grund, map] zurück. */
    function importText(text) {
      ...
      let m = raw.format === FORMAT ? raw.map : raw;
      ...
      gen.normalize(m);
      if (!valid(m)) return [false, "format", null];
      if (!swear.allClean(m.name, m.id, m.author)) return [false, "swear", null];
      if (isFull()) return [false, "full", null];
      m.id = uniqueId(m.id);
      const [ok, reason] = saveMap(m);
      return ok ? [true, "", m] : [false, reason, null];
    }

    return {
      VERSION, MAX_MAPS, EXT, FORMAT, ID_CHARS, MAX_ID, MAX_NAME, MAX_AUTHOR,
      MIN_W, MAX_W, STEP_W, MIN_H, MAX_H, STEP_H, MIN_PAR, MAX_PAR,
      slug, validId, uniqueId, loadMaps, get, count, isFull, saveMap, deleteMap,
      lastAuthor, setLastAuthor, toHole, newMap, exportText, importText,
    };
  })();
